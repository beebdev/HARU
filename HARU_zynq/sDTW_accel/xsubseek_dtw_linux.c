...
typedef struct {
    ...
    XSubseek_dtw_uio_map maps[ MAX_UIO_MAPS ];
} XSubseek_dtw_uio_info;

/***************** Variable Definitions **************************************/
static XSubseek_dtw_uio_info uio_info;
...
static int uio_info_read_name(XSubseek_dtw_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSubseek_dtw_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSubseek_dtw_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSubseek_dtw_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSubseek_dtw_Initialize(XSubseek_dtw *InstancePtr, const char* InstanceName) {
	XSubseek_dtw_uio_info *InfoPtr = &uio_info;
